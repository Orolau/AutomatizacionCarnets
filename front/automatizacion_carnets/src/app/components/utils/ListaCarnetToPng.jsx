...
export default function CarnetToPngConverter({ carnets }) {
    ...
    const descargarLogs = async () => {
        try {
            const res = await fetch("http://localhost:3000/api/errors/errores-personas");
            if (!res.ok) throw new Error("Error al obtener datos del backend");
    
            const data = await res.json();
    
            let logs = `=== INFORME DE ERRORES DE USUARIOS ===\n\n`;
            logs += `Total usuarios con errores: ${data.totalUsuariosConErrores}\n\n`;
    
            data.usuarios.forEach((usuario, i) => {
                logs += `#${i + 1} → ${usuario.nombre} ${usuario.apellidos} (${usuario.correo})\n`;
                logs += `Errores detectados: ${usuario.totalErrores}\n`;
                logs += `Campos faltantes:\n`;
                usuario.camposFaltantes.forEach((campo) => {
                    logs += `   - ${campo}\n`;
                });
                logs += `-------------------------------\n`;
            });
            ...
            saveAs(blob, "informe_errores_carnets.txt");
        } catch (error) {
            console.error("Error al generar logs:", error);
            alert("Ocurrió un error al generar el log de errores.");
        }
    };
